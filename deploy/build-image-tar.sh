...
OUTPUT="$PROJECT_DIR/Share-server-1.2.1-linux-amd64.tar"
BUILD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/share-image.XXXXXX")
trap 'rm -rf "$BUILD_DIR"' EXIT HUP INT TERM

cd "$SERVER_DIR"
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w -X main.version=1.2.1" -o "$BUILD_DIR/nasdrive" .
