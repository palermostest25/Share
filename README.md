...
- The native Mac app browses, uploads, moves, renames, deletes, previews with Space, streams media, and automatically mounts `/Share/` as the `/Volumes/Share` WebDAV volume after a successful personal-account sign-in. Closing the window leaves Share in the menu bar; its background connection check retries a detached Finder mount after the network returns, with a bounded backoff. **Show Share in Finder** and **Reconnect Finder** are available from the menu bar. Enable **Keep Share in the menu bar after login** in Share Settings to start it at Mac login. Finder's sidebar must have **Connected servers** enabled in Finder Settings. The old bootstrap access key cannot mount Finder after the first admin is created; enter your username and password in the Mac app. WebDAV is a mounted volume, not a File Provider extension: Finder cannot serve offline files and Finder/Preview may cache files they open. For a multi-gigabyte video without a local copy, use the Share app or web player.
